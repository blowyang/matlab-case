function [C_Matrix,D_Matrix]=Get_CD_Matrix(K,N,R1,R2,x1,x2,a,km,Gm)
C_Matrix=zeros(8*K,8*N);
D_Matrix=zeros(8*K,4*N);
S1=zeros(N,2*N);
for n=1:N
    S1(n,2*n-1)=1;
    S1(n,2*n)=1i;
...
for k=1:K
    ...
    [L11_zk_AB,L12_zk_AB,L2_zk_AB,L31_zk_AB,L32_zk_AB,...
        L4_zk_AB,L51_zk_AB,L52_zk_AB,L6_zk_AB,~,~,~,~]=Get_L_List(N,R1,R2,x1,x2,a,zk_AB);

    [L11_zk_CD,L12_zk_CD,L2_zk_CD,L31_zk_CD,L32_zk_CD,...
        L4_zk_CD,L51_zk_CD,L52_zk_CD,L6_zk_CD,~,~,~,~]=Get_L_List(N,R1,R2,x1,x2,a,zk_CD);

    [L11_zk_DB,L12_zk_DB,L2_zk_DB,L31_zk_DB,L32_zk_DB,...
        L4_zk_DB,L51_zk_DB,L52_zk_DB,L6_zk_DB,~,~,~,~]=Get_L_List(N,R1,R2,x1,x2,a,zk_DB);

    [L11_zk_CA,L12_zk_CA,L2_zk_CA,L31_zk_CA,L32_zk_CA,...
        L4_zk_CA,L51_zk_CA,L52_zk_CA,L6_zk_CA,~,~,~,~]=Get_L_List(N,R1,R2,x1,x2,a,zk_CA);
    %C_Matrix
